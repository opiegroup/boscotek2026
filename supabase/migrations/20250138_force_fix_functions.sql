-- =============================================================================
-- Force fix remaining function search_path warnings
-- Drop ALL overloads and recreate with proper search_path
-- =============================================================================

-- Use DO blocks to find and drop all versions of each function
DO $$
DECLARE
  func_oid oid;
BEGIN
  -- Drop all versions of has_role
  FOR func_oid IN 
...
DECLARE
  func_oid oid;
BEGIN
  -- Drop all versions of has_any_role
  FOR func_oid IN 
    SELECT p.oid FROM pg_proc p 
    JOIN pg_namespace n ON p.pronamespace = n.oid 
...
DECLARE
  func_oid oid;
BEGIN
  -- Drop all versions of queue_notification
  FOR func_oid IN 
    SELECT p.oid FROM pg_proc p 
    JOIN pg_namespace n ON p.pronamespace = n.oid 
...
DECLARE
  func_oid oid;
BEGIN
  -- Drop all versions of queue_notification_for_roles
  FOR func_oid IN 
    SELECT p.oid FROM pg_proc p 
    JOIN pg_namespace n ON p.pronamespace = n.oid 
    WHERE n.nspname = 'public' AND p.proname = 'queue_notification_for_roles'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', func_oid::regprocedure);
  END LOOP;
END $$;
...
-- 4. queue_notification_for_roles
